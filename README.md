...
| `docs/hardware/10-pcb-design-kicad10.{html,pdf}` | KiCad 10.0 guide (rev 3.4, 76 pp), one section per module — footprints (every capacitor marked ceramic/unpolarized or electrolytic/polarized, verified KiCad symbol names), schematic (USB-C's dual D+/D- pins and ESD IC pinout verified against real datasheets), direct pin-to-pin connections, and programming/debug header wiring — for the custom 4-layer field PCB, through layer stack, copper pours, DRC, gerber export, JLCPCB upload. |
