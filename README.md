...
| `docs/hardware/10-pcb-design-kicad10.{html,pdf}` | KiCad 10.0 guide (rev 3.7, 79 pp), one section per module — footprints (every capacitor marked ceramic/unpolarized or electrolytic/polarized, verified KiCad symbol/footprint pairings including RJ45 and USB-C), schematic (RJ45's real internal pinout with correct active-low LED polarity; USB-C as one linear connector → ESD clamp → resistor → MCU diagram), direct pin-to-pin connections, and programming/debug header wiring — for the custom 4-layer field PCB, through layer stack, copper pours, DRC, gerber export, JLCPCB upload. |
