...
| `docs/hardware/10-pcb-design-kicad10.{html,pdf}` | KiCad 10.0 guide (rev 3.2, 76 pp), one section per module — footprints (every capacitor marked ceramic/unpolarized or electrolytic/polarized), schematic (USB-C ESD IC pinout verified against ST's datasheet), direct pin-to-pin connections, and programming/debug header wiring — for the custom 4-layer field PCB, through layer stack, copper pours, DRC, gerber export, JLCPCB upload. |
