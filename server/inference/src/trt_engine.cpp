...
spdlog::level::level_enum to_spdlog(nvinfer1::ILogger::Severity s) noexcept {
    ...
    switch (s) {
        case S::kINTERNAL_ERROR: return spdlog::level::critical;
        case S::kERROR:          return spdlog::level::err;
        case S::kWARNING:        return spdlog::level::warn;
        case S::kINFO:           return spdlog::level::info;
        case S::kVERBOSE:        return spdlog::level::debug;
    }
...
std::size_t element_size(nvinfer1::DataType dt) noexcept {
    ...
    switch (dt) {
        ...
        case DT::kINT4:  return 1;  // packed: caller must round up
        case DT::kFP4:   return 1;  // packed: caller must round up
        case DT::kE8M0:  return 1;
    }
    return 0;
}
...
std::size_t volume(const nvinfer1::Dims& d) noexcept {
    ...
    for (int i = 0; i < d.nbDims; ++i) {
        if (d.d[i] < 0) return 0;  // dynamic dim; caller must resolve first
...
TrtEngine TrtEngine::load(const std::filesystem::path& engine_path,
    ...
    spdlog::info("Loaded TensorRT engine: {} ({} I/O tensors)",
                 engine_path.string(), self.tensors_.size());
    return self;
}
...
void TrtEngine::allocate_buffers_() {
    ...
    if (!any_dynamic_outputs) return;

    for (const auto& io : tensors_) {
        if (!io.is_input) continue;
        const bool has_dynamic =
            std::any_of(io.shape.d, io.shape.d + io.shape.nbDims, [](int64_t x){ return x < 0; });
    ...
    for (std::size_t i = 0; i < tensors_.size(); ++i) {
        if (device_buffers_[i]) continue;  // already allocated
        const auto& io   = tensors_[i];
